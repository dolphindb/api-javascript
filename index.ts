...
export class DDB {
    ...
    */
    async rpc <T extends DdbObj = DdbObj> (
        ...
        const buf_data = await presult
        
        if (this.print_buffer)
            console.log(
                typed_array_to_buffer(buf_data)
            )
        
        // LOCAL: 便于 repl
        // return global.DdbObj.parse(data_buf) as DdbObj
        
        return DdbObj.parse(buf_data, this.le) as T
    }
